...
'use strict';
const URL = "http://ebaer.se/wd3/projekt/REST.php/Lorem_Ipsum";
...
class REST_Lorem{
    ...
    function newLorem(loremname, ipsumtext){
        let liname = loremname;
        let litext = ipsumtext;
    ...
    function changeLorem(loremid, loremname, loremtext){
        ...
        let liname = loremname;
        //Remove linebreaks
        let litext = this.LoremBreak(loremtext);
...
//if (LI !== undefined){
    ...
    //Check if on handle.html by looking for envelophandle div
        ...
        function newIpsum(){
            let liname = document.getElementById("liname").value;
            let litext = document.getElementById("litext").value;
        ...
        function changeIpsum(liid, liname,litext){
            let loremid = liid;
            let loremname = liname;
            let loremtext = LoremRestore(litext);
